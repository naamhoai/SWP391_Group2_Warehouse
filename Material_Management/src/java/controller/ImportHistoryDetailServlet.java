 ...
 * @author kien3
 */
@WebServlet(name="ImportHistoryDetailServlet", urlPatterns={"/ImportHistoryDetailServlet"})
public class ImportHistoryDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<ImportHistory> importDetailList = dao.getImportHistoryDetail(importId);
        ImportHistory importHistory = null;
        if (!importDetailList.isEmpty()) {
            importHistory = importDetailList.get(0);
        }
        request.setAttribute("importHistory", importHistory);
        request.setAttribute("importDetailList", importDetailList);
        request.getRequestDispatcher("ImportHistoryDetailServlet.jsp").forward(request, response);
    } 
