 ...
 * @author kimoa
 */
@WebServlet(name = "ApproveRequestServlet", urlPatterns = {"/approveRequest"})
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (warehouseStaffId != -1) {
                ...
                System.out.println("Không tìm thấy nhân viên kho!");
            }

            // Chuyển hướng về trang giám đốc
            response.sendRedirect(request.getContextPath() + "/director");

        } catch (Exception e) {
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Lỗi khi phê duyệt yêu cầu: " + e.getMessage());
        }
    }
